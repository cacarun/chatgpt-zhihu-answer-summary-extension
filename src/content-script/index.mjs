...
async function run(question) {
  ...
  port.onMessage.addListener(function (msg) {
    ...
    `;
      ...
      container.innerHTML = `<div class='chat-gpt-zhihu-header'>Please Login <a href="https://chat.openai.com" target="_blank" style="text-decoration: underline;">chat.openai.com</a> First!</div>`;
    }  else if (msg.error.includes("network error")) {
      container.innerHTML = `<div class='chat-gpt-zhihu-header'>ChatGPT network error, Please Click Try!</div>`;
    } else {
      if (answer === "") {
        container.innerHTML =
          "<div class='chat-gpt-zhihu-header'>Failed to get response from ChatGPT</div>";
      }
    }
  });
